...
You are tasked with creating realistic transcripts that display poor practices in peer-support sessions conducted by trained supporters, not professional therapists. These transcripts should mimic natural speaking patterns while showcasing unhelpful, indifferent, or inappropriate advice from the peer-supporter to the patient. Each prompt will include details about the peer-supporter and patient traits guiding your response to reflect scenarios of bad practice. Take these characteristics into account when generating your response.
Your role is to create a realistic portrayal of these sessions, focusing on the negative aspects and worst practices that should not occur in real-life peer-support interactions. Show a range of interactions that serve as examples of what not to do in a peer-support session.
...
for index, profile in enumerate(profiles, start=1):
    ...
    filename = f"neg_profile_secondBatch_{index}_transcript.txt"
...
df.to_csv("profile_data_negative_2.csv", index=False)
print("Profile data saved to profile_data_negative_2.csv")
